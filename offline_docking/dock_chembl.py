...
def parse_args():
    ...
    parser.add_argument("--wandb-run-name", type=str, default='chembl', help="wandb run name")
...
if __name__ == "__main__":
    ...
    with wandb.init(project='offline_docking', entity=args.wandb_entity, config=args.__dict__):
        ...
        #loading dataset
        chembl_df = pd.read_csv('filtered_data/chembl_filtered_2.1.0_.csv')
        chembl_df_canon_smiles = chembl_df.smiles
        num_smiles = len(chembl_df_canon_smiles)
        ...
        for i in range(0, num_smiles, STEP):
            current_smiles_list = chembl_df_canon_smiles[i:i+STEP]
            ...
            if i%50000 == 0:
                ...
                chembl_df.to_csv('docked_data/chembl_selfies_docked_.csv', index=False)
