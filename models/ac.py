...
class RnnValue(nn.Module):
    ...
    def forward(self, x, lengths, hiddens=None):
        ...
        return values, lengths, hiddens

    def get_value(self, obs, smiles_len, nonterms):      
    ...
    def load_save_dict(self, saved_dict, critic=False):
        ...
        if critic:
            self.linear.load_state_dict(saved_dict["critic"])
...
class RnnPolicy(nn.Module):
    ...
    def get_data(self, critic, batch_size, max_length, device):
        ...
        with torch.no_grad():
            values, _, _ = critic(obs[:-1], episode_lens)
            values = values.squeeze(-1) * nonterms[:-1]

        return obs, rewards, logprobs, values, nonterms, episode_lens
